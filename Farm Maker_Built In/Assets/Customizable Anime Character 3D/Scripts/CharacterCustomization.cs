    ...
    public class CharacterCustomization : MonoBehaviour
    {
        ...
        */
        ...
        public void ApplyAccessory(int x)
        {
            ...
            if (accessories.Count > x && accessories[x] != null && accessories[x].AccessoryObject != null)
            {
                var temp = accessories[x].AccessoryObject;
                var accComp = temp.GetComponent<Accessory>();
                if (accComp && accComp.armature) SafeDestroy(accComp.armature.gameObject);
                SafeDestroy(temp);
            }

            if (accessories.Count > x && accessories[x] != null && accessories[x].Accessory != null)
            {
                GameObject sceneAcc = EditorInstantiatePrefab(accessories[x].Accessory.Accessory, this.transform);
                sceneAcc.transform.localPosition = Vector3.zero;

                EditorUnpack(sceneAcc);

                var acc = sceneAcc.GetComponent<Accessory>();
                if (acc != null)
                {
                    if (acc.armature) acc.armature.transform.SetParent(armheadRef.transform, false);
                    var smr = acc.skin ? acc.skin.GetComponent<SkinnedMeshRenderer>() : null;
                    if (smr) smr.material = body;
                }

                accessories[x].AccessoryObject = sceneAcc;
                accessories[x].AltIndex = 0;

                if (accessories[x].Accessory.ColorVar != null && accessories[x].Accessory.ColorVar.Length > 0)
                {
                    ChangeAccessoryColor(x);
                }
            }
            ...
            {
                Debug.LogWarning("ApplyAccessory: accessories[x]나 Accessory가 비어있습니다.");
            }
        }
        ...
        */
        ...
        public void ApplyChange(string ClothesID)
        {
            ...
            if (ClothesID == "HairBack")
            {
                ...
                if (Backhairs != null)
                {
                    var acc = Backhairs.GetComponent<Accessory>();
                    if (acc && acc.armature) SafeDestroy(acc.armature.gameObject);
                    SafeDestroy(Backhairs);
                    Backhairs = null;
                }

                if (BackHair != null)
                {
                    GameObject sceneHair = SafeInstantiate(BackHair.Hair, this.transform);
                    var acc = sceneHair.GetComponent<Accessory>();
                    if (acc)
                    {
                        if (acc.armature) acc.armature.transform.SetParent(armheadRef.transform, false);
                        var smr = acc.skin?.GetComponent<SkinnedMeshRenderer>();
                        if (smr)
                        {
                            smr.material = body;
                            smr.sharedMesh = BackHair.ColorVar[0].items;
                        }
                    }
                    Backhairs = sceneHair;
                    SafeRecordPrefab(this.gameObject);
                }
            }
            ...
            else if (ClothesID == "HairFront")
            {
                ...
                if (Fronthairs != null)
                {
                    var acc = Fronthairs.GetComponent<Accessory>();
                    if (acc && acc.armature) SafeDestroy(acc.armature.gameObject);
                    SafeDestroy(Fronthairs);
                    Fronthairs = null;
                }

                if (FrontHair != null)
                {
                    GameObject sceneHair = SafeInstantiate(FrontHair.Hair, this.transform);
                    var acc = sceneHair.GetComponent<Accessory>();
                    if (acc)
                    {
                        if (acc.armature) acc.armature.transform.SetParent(armheadRef.transform, false);
                        ...
                        {
                            smr.material = body;
                            smr.sharedMesh = FrontHair.ColorVar[0].items;
                        }
                    }
                    Fronthairs = sceneHair;
                    SafeRecordPrefab(this.gameObject);
            ...
            else if (ClothesID == "HairSide")
            {
                ...
                if (Sidehairs != null)
                {
                    var acc = Sidehairs.GetComponent<Accessory>();
                    if (acc && acc.armature) SafeDestroy(acc.armature.gameObject);
                    SafeDestroy(Sidehairs);
                    Sidehairs = null;
                }

                if (SideHair != null)
                {
                    GameObject sceneHair = SafeInstantiate(SideHair.Hair, this.transform);
                    var acc = sceneHair.GetComponent<Accessory>();
                    if (acc)
                    {
                        if (acc.armature) acc.armature.transform.SetParent(armheadRef.transform, false);
                        ...
                        {
                            smr.material = body;
                            smr.sharedMesh = SideHair.ColorVar[0].items;
                        }
                    }
                    Sidehairs = sceneHair;
                    SafeRecordPrefab(this.gameObject);
            ...
            // ... (Pants, Jacket, Shoes 등 기존 로직 동일하게 유지)
        ...
        private static GameObject EditorInstantiatePrefab(GameObject prefab, Transform parent)
        {
            ...
            return go;
#else
// 런타임 대체: 그냥 Instantiate
var go = GameObject.Instantiate(prefab, parent, false);
return go;
#endif
        }

        private static void EditorUnpack(GameObject go)
        {
#if UNITY_EDITOR
        ...
        private static void EditorRecord(UnityEngine.Object obj)
        {
#if UNITY_EDITOR
...
UnityEngine.Object.Destroy(obj);
    ...
    // 안전한 Instantiate (Editor/Runtime 겸용)
private GameObject SafeInstantiate(GameObject prefab, Transform parent)
        {
#if UNITY_EDITOR
    ...
    UnityEngine.Object.Destroy(obj);
        ...
        // Prefab 레코드 (Editor 전용)
        private void SafeRecordPrefab(UnityEngine.Object obj)
        {
#if UNITY_EDITOR
            if (Application.isEditor)
